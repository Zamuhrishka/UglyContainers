...
static void uqueue_uint8_main_test(void **state)
{
	...
	uqueue = uqueue_create(sizeof(inputData), sizeof(uint8_t), compare_uint8);
	...
	{
		assert_true(uqueue_enqueue(uqueue, &i));
	}

	assert_int_equal(uqueue_size(uqueue), BUFFER_SIZE);
...
static void uqueue_uint16_main_test(void **state)
{
	...
	uqueue = uqueue_create(sizeof(inputData), sizeof(uint16_t), compare_uint16);
	assert_int_not_equal(uqueue, NULL);
	...
	assert_int_equal(uqueue_size(uqueue), BUFFER_SIZE/sizeof(uint16_t));
	...
	assert_memory_equal(inputData, outputData, BUFFER_SIZE/sizeof(uint16_t));
}

static void uqueue_uint32_main_test(void **state)
{
	uint32_t outputData[QUEUE_SIZE/sizeof(uint32_t)] = {0};
	uint32_t inputData[QUEUE_SIZE/sizeof(uint32_t)] = {0};
	...
	uqueue = uqueue_create(QUEUE_SIZE, sizeof(uint32_t), compare_uint32);
	assert_int_not_equal(uqueue, NULL);
	...
	assert_int_equal(uqueue_size(uqueue), BUFFER_SIZE/sizeof(uint32_t));
	...
	assert_memory_equal(inputData, outputData, BUFFER_SIZE/sizeof(uint32_t));
}

static void uqueue_struct_main_test(void **state)
{
	template_t outputData[QUEUE_SIZE/sizeof(template_t)] = {0};
	template_t inputData[QUEUE_SIZE/sizeof(template_t)] = {0};
	...
	uqueue = uqueue_create(QUEUE_SIZE, sizeof(template_t), compare_template_t);
	assert_int_not_equal(uqueue, NULL);

	for(size_t i = 0; i < BUFFER_SIZE/sizeof(template_t); i++)
	{
		assert_true(uqueue_enqueue(uqueue, (void*)&inputData[i]));
	}
	...
	assert_int_equal(uqueue_size(uqueue), BUFFER_SIZE/sizeof(template_t));

	for(size_t i = 0; i < BUFFER_SIZE/sizeof(template_t); i++)
	{
		assert_true(uqueue_denqueue(uqueue, (void*)&outputData[i]));
	}
